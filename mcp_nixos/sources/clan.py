...
from ..caches import clan_cache
from ..config import APIError
from ..utils import error, strip_html


def _search_clan(query: str, limit: int) -> str:
    """Search Clan options from NuschtOS meta JSON."""
    try:
        options = clan_cache.get_options()
        ...
        if not matches:
            return f"No Clan options found matching '{query}'"

        results = [f"Found {len(matches)} Clan options matching '{query}':\n"]
        for opt in matches:
...
def _info_clan(name: str) -> str:
    """Get detailed info for a Clan option."""
    try:
        options = clan_cache.get_options()
        ...
        for opt in options:
            if opt.get("name") == name:
                return _format_clan_option(opt)
        ...
        return error(f"Clan option '{name}' not found", "NOT_FOUND")
    except APIError:
        raise
    except Exception as e:
        return error(str(e))
...
def _format_clan_option(opt: dict[str, Any]) -> str:
    """Format a Clan option for detailed display."""
    lines = [f"Clan Option: {opt.get('name', '')}"]
...
def _browse_clan_options(prefix: str) -> str:
    """Browse Clan options by prefix, or list categories if no prefix."""
    try:
        options = clan_cache.get_options()
        ...
        if not prefix:
            ...
            results = [f"Clan option categories ({len(categories)} total):\n"]
        ...
        if not matches:
            return f"No Clan options found with prefix '{prefix}'"

        results = [f"Clan options with prefix '{prefix}' ({len(matches)} found):\n"]
